    ...
    <div className="max-w-2xl mx-auto px-4 py-10">
      <div className="mb-6">
        ...
        <p className="text-sm text-slate-500 mt-1">
          Answer all 3 questions. Score 2/3 or better to unlock next topics.
        </p>
      </div>
      ...
      {!result ? (
        <>
          ...
          >
            {submitting ? (
              ...
              <>
                <ArrowRight size={16} /> Submit Answers
              </>
            )}
          </button>
        </>
        ...
        <div>
          <div
            className={`rounded-2xl p-6 mb-6 ${result.passed ? "bg-green-50 border border-green-200" : "bg-red-50 border border-red-200"}`}
          >
            ...
            <p className="text-sm text-slate-600">
              Score: <strong>{result.score}/3</strong> · Mastery:{" "}
              <strong>{(result.p_know * 100).toFixed(0)}%</strong>
            </p>
            {result.newly_unlocked.length > 0 && (
              <p className="text-sm text-green-700 mt-2 font-medium">
                Unlocked: {result.newly_unlocked.join(", ")}
              </p>
            )}
          </div>
          ...
          <div className="space-y-4 mb-6">
            {result.results.map((r, i) => (
              <div
                key={i}
                className={`rounded-xl border p-4 ${r.is_correct ? "border-green-200 bg-green-50/50" : "border-red-200 bg-red-50/50"}`}
              >
          ...
          {result.resource_suggestion && (
            <div className="bg-blue-50 border border-blue-200 rounded-xl p-4 mb-6">
              ...
              <p className="text-xs text-blue-600 mb-3">
                {result.resource_suggestion.message}
              </p>
              <div className="flex gap-2 flex-wrap">
                {[
                  ["YouTube Tutorial", result.resource_suggestion.youtube],
                  ["MDN Docs", result.resource_suggestion.mdn],
                ].map(([label, url]) => (
                  <a
                    key={label}
                    href={url}
          ...
          <div className="flex gap-3">
            <button
              onClick={loadQuiz}
              className="flex items-center gap-2 px-5 py-2.5 border border-slate-200 rounded-xl text-sm font-medium text-slate-700 hover:bg-slate-50 transition-colors"
            >
