  ...
  useEffect(() => {
    Promise.all([getGraphData(), getMyProgress()]).then(([gRes, pRes]) => {
      ...
      const nodes = gRes.data.nodes.map((n) => ({
        ...
        val:
          (prog[n.id]?.status === "completed" ? 2 : 1) * (6 - n.difficulty) + 3,
      }));
        ...
        {selected ? (
          <div>
              ...
              <div className="flex justify-between text-sm">
                ...
                <span
                  className={`font-medium capitalize ${
                    progress[selected.id]?.status === "completed"
                      ? "text-teal-600"
                      : progress[selected.id]?.status === "unlocked"
                        ? "text-blue-600"
                        : "text-slate-400"
                  }`}
                >
                  {progress[selected.id]?.status || "locked"}
                </span>
              </div>
              {progress[selected.id] && (
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-medium text-teal-700">
                    {(progress[selected.id].p_know * 100).toFixed(0)}%
                  </span>
                </div>
              )}
            </div>
            {(progress[selected.id]?.status === "unlocked" ||
              progress[selected.id]?.status === "completed") && (
              <button
                onClick={() => navigate(`/quiz/${selected.id}`)}
