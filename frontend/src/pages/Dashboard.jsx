...
export default function Dashboard() {
  ...
  const chartData = [
    { name: "Progress", value: data.completion_pct, fill: "#0D7377" },
  ];
    ...
    <div className="max-w-5xl mx-auto px-4 py-8">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-8">
        {[
          ...
          },
          {
            ...
            icon: <Unlock size={20} className="text-blue-600" />,
            color: "bg-blue-50",
          },
          {
            label: "Total Topics",
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-white rounded-2xl border border-slate-200 p-6 flex flex-col items-center justify-center">
          ...
          <div className="text-3xl font-bold text-teal-700 -mt-4">
            {data.completion_pct}%
          </div>
          <div className="text-xs text-slate-400 mt-1">
            {data.completed_count} of {data.total_topics} topics
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl border border-slate-200 p-6">
          ...
          {data.confused_topics.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-28 text-slate-300">
              ...
              <p className="text-sm mt-2">All good!</p>
            </div>
          ) : (
            <div className="space-y-2">
              {data.confused_topics.map((t) => (
                <div
                  key={t.topic_id}
                  className="flex items-start gap-2 p-3 bg-orange-50 rounded-xl"
                >
                  <AlertTriangle
                    size={15}
                  ...
                  <div>
                    ...
                    <div className="text-xs text-orange-600">
                      {t.attempts} attempts · P(know)={t.p_know}
                    </div>
                  </div>
                </div>
              ))}
      ...
      <div className="mt-6 bg-white rounded-2xl border border-slate-200 p-6">
        <h2 className="text-sm font-semibold text-slate-500 mb-4 uppercase tracking-wide">
          Unlocked Topics — Ready to Learn
        </h2>
        {data.unlocked_topics.length === 0 ? (
          <p className="text-sm text-slate-400">
            Complete topics to unlock more!
          </p>
        ) : (
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
            {data.unlocked_topics.map((t) => (
              ...
              >
                <Unlock
                  size={14}
                  className="text-slate-400 group-hover:text-teal-600 flex-shrink-0"
                />
                <span className="text-sm font-medium text-slate-700 group-hover:text-teal-800">
                  {t.name}
                </span>
              </button>
            ))}
            ...
            {data.recent_attempts.map((a, i) => (
              ...
              >
                ...
                <div className="flex items-center gap-3">
                  ...
                  <span
                    className={`text-xs px-2 py-0.5 rounded-full font-medium ${a.passed ? "bg-green-100 text-green-700" : "bg-red-100 text-red-600"}`}
                  >
